...
export class SnapshotService {
  ...
  async createQuerySnapshot(dto: CreateQuerySnapshotDto) {
    ...
    const querySnapshot: QuerySnapshot = {
      ...
      cacheExpireMinutes: dto.cacheExpireMinutes,
      isDemo: dto.isDemo || false,
      timestamp,
      createdAt: new Date().toISOString(),
    };
  ...
  async executeQuerySnapshot(id: string): Promise<Snapshot> {
    ...
    // 檢查快取是否過期（如果是快取模式）
    if (querySnapshot.snapshotMode === 'cached' && querySnapshot.lastUpdated) {
      const lastUpdateTime = new Date(querySnapshot.lastUpdated).getTime();
      const cacheExpireTime =
        (querySnapshot.cacheExpireMinutes || 60) * 60 * 1000;

      if (Date.now() - lastUpdateTime < cacheExpireTime) {
        // 快取未過期，返回快取資料（這裡需要實作快取資料的讀取）
        // TODO: 實作快取資料讀取邏輯
      }
    }

    // 如果是動態模式或快取已過期，執行即時查詢
    if (
      querySnapshot.snapshotMode === 'dynamic' ||
      querySnapshot.snapshotMode === 'cached'
    ) {
      try {
        // 解密 API Token
        ...
        if (querySnapshot.snapshotMode === 'cached') {
          querySnapshot.lastUpdated = new Date().toISOString();
          const filePath = join(this.querySnapshotDir, `${id}.json`);
          await fs.writeFile(filePath, JSON.stringify(querySnapshot, null, 2));
        }
