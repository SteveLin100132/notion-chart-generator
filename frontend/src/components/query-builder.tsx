...
export interface FilterCondition {
  ...
  property: string;
  operator: string;
  value?: string | number | boolean | string[]; // 支援陣列值，用於 multi_select 的 contains/does_not_contain
...
export const QueryBuilder: React.FC<QueryBuilderProps> = ({
  ...
  const [groups, setGroups] = useState<FilterGroup[]>(
    value.length > 0 ? value : [createNewGroup()]
  );
  ...
  const getOperatorsForProperty = useCallback(
    (propertyName: string): OperatorConfig[] => {
      const property = properties.find((p) => p.name === propertyName);
      // 若找不到 property，但 propertyName 字串包含 multi_select，仍回傳 multi_select 運算子
      if (!property) {
        if (
          propertyName &&
          propertyName.toLowerCase().includes("multi_select")
        ) {
      ...
      const newGroups = groups.map((group) => {
        if (group.id === groupId) {
          ...
          return {
            ...group,
            conditions:
              newConditions.length > 0 ? newConditions : [createNewCondition()],
          };
        }
        return group;
      });
  ...
  // 添加子群組到指定群組
  const addSubgroupToGroup = useCallback(
  ...
  // 從群組中刪除子群組
  const removeSubgroupFromGroup = useCallback(
    (parentGroupId: string, subgroupId: string) => {
      const removeSubgroupRecursively = (group: FilterGroup): FilterGroup => {
        if (group.id === parentGroupId) {
          return {
            ...group,
            subgroups:
              group.subgroups?.filter((sg) => sg.id !== subgroupId) || [],
          };
        }
  ...
  // 遞歸更新子群組的邏輯運算符
  const updateSubgroupLogicalOperator = useCallback(
    (subgroupId: string, operator: "and" | "or") => {
  ...
  // 遞歸添加條件到子群組
  const addConditionToSubgroup = useCallback(
    (subgroupId: string) => {
  ...
  // 遞歸從子群組中刪除條件
  const removeConditionFromSubgroup = useCallback(
    (subgroupId: string, conditionId: string) => {
    ...
    (condition: FilterCondition, groupId: string) => {
      ...
      if (
        ...
        if (!op) {
          ...
          setTimeout(
            () =>
              updateCondition(groupId, condition.id, { operator: "contains" }),
            0
          );
        }
        ...
        if (multiSelectOps.includes(op)) {
          ...
          if (Array.isArray(condition.value)) {
            selectedValues = condition.value.filter(
          ...
          return (
            <div style={{ maxWidth: 320 }}>
              <MultiSelectDropdown
                ...
                onChange={(vals) =>
                  updateCondition(groupId, condition.id, { value: vals as any })
                }
                ...
                <SelectValue placeholder="選擇選項">
                  {condition.value &&
                    property?.options?.find(
                      (option) => option.name === condition.value
                    ) && (
                      <Badge
      ...
      // 其餘型別
      const operator = getOperatorsForProperty(condition.property).find(
      ...
      switch (property?.type) {
        ...
        case "date":
          if (condition.operator === "between") {
            ...
            const start = condition.value
              ? new Date(condition.value as string)
              : null;
            const end = condition.endValue
              ? new Date(condition.endValue as string)
              : null;
      ...
      return (
        <div
          ...
          className={`border rounded-lg p-4 ${
            isSubgroup
              ? "border-gray-300 bg-gray-25 ml-6"
...
export const convertToNotionFilter = (
  groups: FilterGroup[],
  properties: DatabaseProperty[] = []
): any => {
  ...
  const convertCondition = (condition: FilterCondition): any => {
    ...
    console.log("convertCondition called with:", {
      property,
      operator,
      value,
      type: typeof value,
    ...
    // select/multi_select 欄位複選: 轉換為 or/and equals/does_not_equal 子條件
    ...
    if (
      ...
      console.log("Entering array conversion for select/multi_select");
      // 對於 select 和 multi_select，使用不同的子操作符
      let subOperator: string;
      if (propertyType === "select") {
        // select 類型使用 equals/does_not_equal
        subOperator = operator === "contains" ? "equals" : "does_not_equal";
      } else {
        // multi_select 類型使用 contains/does_not_contain，但每個值單獨處理
        subOperator = operator === "contains" ? "contains" : "does_not_contain";
      }
      ...
      console.log("Array conversion result:", { [logic]: subFilters });
      return { [logic]: subFilters };
    }

    // 特殊處理：對於 multi_select 的 contains/does_not_contain，如果值不是陣列，也要特殊處理
    if (
      propertyType === "multi_select" &&
      (operator === "contains" || operator === "does_not_contain")
    ) {
      console.log("Entering fallback conversion for multi_select");
      // 如果值不是陣列，轉換為陣列處理
      const valueArray = Array.isArray(value) ? value : value ? [value] : [];
      if (valueArray.length > 0) {
        // multi_select 使用 contains/does_not_contain
        const subOperator =
          operator === "contains" ? "contains" : "does_not_contain";
        const logic = operator === "contains" ? "or" : "and";
        const subFilters = valueArray.map((v) => ({
          property,
          [propertyType]: { [subOperator]: v },
        }));
        console.log("Fallback conversion result:", { [logic]: subFilters });
        return { [logic]: subFilters };
      }
    }
    // 其餘型別
    ...
    switch (operator) {
      ...
      case "does_not_equal":
        ...
        break;
      case "contains":
        // 對於 multi_select，contains 應該已經在上面處理了，這裡只處理其他類型
        if (propertyType !== "multi_select") {
          filterValue.contains = value;
        }
        break;
      case "does_not_contain":
        // 對於 multi_select，does_not_contain 應該已經在上面處理了，這裡只處理其他類型
        if (propertyType !== "multi_select") {
    ...
    // 檢查 filterValue 是否為空物件，如果是則返回 null
  ...
  const convertGroup = (group: FilterGroup): any => {
    ...
    const validSubgroups = group.subgroups
      ? group.subgroups.map(convertGroup).filter(Boolean)
      : [];
